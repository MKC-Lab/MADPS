...
class CustomDPOTrainer(DPOTrainer):
    ...
    def asft_loss(self, chosen_logps: "torch.Tensor", rejected_logps: "torch.Tensor") -> "torch.Tensor":
        r"""Compute ASFT's odds ratio (OR) loss for batched log probabilities of the policy model."""
        align_loss = -torch.log(torch.exp(chosen_logps)) - torch.log1p(-torch.exp(rejected_logps))
        sft_loss = -chosen_logps
        asft_loss = sft_loss + self.beta * align_loss
        return asft_loss
    ...
    def compute_preference_loss(
        ...
        if not self.finetuning_args.use_ref_model:
            ...
            elif self.loss_type == "asft":
                losses = self.asft_loss(policy_chosen_logps, policy_rejected_logps)
    ...
    def concatenated_forward(
        ...
        if self.loss_type in ["ipo", "orpo", "asft", "simpo"]:
    ...
    def get_batch_loss_metrics(
        ...
        if self.loss_type == "orpo" or self.loss_type == "asft":
